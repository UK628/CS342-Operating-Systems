        ...
        switch (opt) {
            ...
            case 'a':
                if (strlen(optarg) == 1 || strlen(optarg) == 3) {
                    ...
                    if (strlen(queue_selection_method) == 2 && (queue_approach == 'M') && ((queue_selection_method[0] == 'N'  && queue_selection_method[1] == 'A') || ((queue_selection_method[0] == 'L' || queue_selection_method[0] == 'R') && (queue_selection_method[1] == 'M') ))) {
...
void init_ready_queue(struct ReadyQueue* rq, char* algorithm, int time_quantum) {
    ...
    pthread_mutex_init(&rq->cond_lock, NULL);
...
void* simulate(void* param) {
    ...
    while(1) {      
        pthread_mutex_lock(&rq->lock);
        while(rq->size == 0) {
            pthread_cond_wait(&rq->cv, &rq->lock);
        }
        ...
        if(rq->head != NULL &&  rq->size == 1 && rq->head->pcb.pid == -2) {
            pthread_mutex_unlock(&rq->lock);
            pthread_exit(0);
        } 
        pthread_mutex_unlock(&rq->lock);
