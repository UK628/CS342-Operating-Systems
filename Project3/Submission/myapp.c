#include <unistd.h>
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <pthread.h>
#include <stdarg.h>
#include "rm.h"

#define NUMR 5        // number of resource types
#define NUMP 5        // number of threads

int AVOID = 0;
int random_func = 0;
int exist[5] =  {10, 10, 10, 10, 10};  // resources existing in the system
int max_val = 10;

void pr (int tid, char astr[], int m, int r[])
{
    int i;
    printf ("thread %d, %s, [", tid, astr);
    for (i=0; i<m; ++i) {
        if (i==(m-1))
            printf ("%d", r[i]);
        else
            printf ("%d,", r[i]);
    }
    printf ("]\n");
}


void setarray (int r[MAXR], int m, ...)
{
    va_list valist;
    int i;

    va_start (valist, m);
    for (i = 0; i < m; i++) {
        r[i] = va_arg(valist, int);
    }
    va_end(valist);
    return;
}

int* generate_random_numbers(int N, int M) {
    // Initialize random number generator
    srand(time(NULL));
    
    // Allocate memory for the array
    int* numbers = (int*)malloc(N * sizeof(int));
    
    // Generate N random numbers between 0 and M
    for (int i = 0; i < N; i++) {
        numbers[i] = rand() % (M + 1);
    }
    
    return numbers;
}
void *randomFunc(void *a)
{
    int tid;
    int claim[MAXR];
    int *request1;
    int *request2;
    int m = NUMR;

    tid = *((int*)a);
    rm_thread_started(tid);

    // generate random claim
    setarray(claim, m, max_val, max_val, max_val, max_val, max_val);
    rm_claim(claim);

    // generate random request1
    request1 = generate_random_numbers(m, max_val);
    pr(tid, "REQ", m, request1);
    rm_request(request1);

    sleep(4);

    // generate random request2
    request2 = generate_random_numbers(m, max_val);
    pr(tid, "REQ", m, request2);
    rm_request(request2);

    rm_release(request1);
    rm_release(request2);

    free(request1);
    free(request2);
    rm_thread_ended();
    printf("Thread %d is ended! \n", tid);
    pthread_exit(NULL);
}

void *threadfunc1 (void *a)
{
    int tid;
    int request1[MAXR];
    int request2[MAXR];
    int claim[MAXR];

    tid = *((int*)a);
    rm_thread_started (tid);

    setarray(claim, NUMR, 10,10,10,10, 10);
    rm_claim (claim);

    setarray(request1, NUMR, 5, 3, 2, 4, 1);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(4);

    setarray(request2, NUMR, 3, 2, 1, 2, 1);
    pr (tid, "REQ", NUMR, request2);
    rm_request (request2);

    rm_release (request1);
    rm_release (request2);

    rm_thread_ended();
    printf("Thread %d is ended! \n", tid);
    pthread_exit(NULL);
}


void *threadfunc2 (void *a)
{
    int tid;
    int request1[MAXR];
    int request2[MAXR];
    int claim[MAXR];

    tid = *((int*)a);
    rm_thread_started (tid);

    setarray(claim, NUMR, 10,10,10,10, 10);
    rm_claim (claim);

    setarray(request1, NUMR, 3, 1, 2, 2, 3);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(4);

    setarray(request2, NUMR, 1, 0, 0, 2, 1);
    pr (tid, "REQ", NUMR, request2);
    rm_request (request2);

    rm_release (request1);
    rm_release (request2);

    rm_thread_ended();
    printf("Thread %d is ended! \n", tid);
    pthread_exit(NULL);
}

void *threadfunc3 (void *a)
{
    int tid;
    int request1[MAXR];
    int request2[MAXR];
    int claim[MAXR];

    tid = *((int*)a);
    rm_thread_started (tid);

    setarray(claim, NUMR, 10,10,10,10, 10);
    rm_claim (claim);

    setarray(request1, NUMR, 2, 6, 1, 2, 1);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(4);
    rm_release (request1);

    setarray(request2, NUMR, 3, 2, 1, 2, 1);
    pr (tid, "REQ", NUMR, request2);
    rm_request (request2);
    rm_release (request2);

    rm_thread_ended();
    printf("Thread %d is ended! \n", tid);
    pthread_exit(NULL);
}

void *threadfunc4 (void *a)
{
    int tid;
    int request1[MAXR];
    int request2[MAXR];
    int claim[MAXR];

    tid = *((int*)a);
    rm_thread_started (tid);

    setarray(claim, NUMR, 10,10,10,10, 10);
    rm_claim (claim);

    setarray(request1, NUMR, 2, 1, 4, 5, 2);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(4);
    rm_release (request1);

    setarray(request2, NUMR, 3, 2, 1, 2, 1);
    pr (tid, "REQ", NUMR, request2);
    rm_request (request2);
    rm_release (request2);

    rm_thread_ended();
    printf("Thread %d is ended! \n", tid);
    pthread_exit(NULL);
}

void *threadfunc5 (void *a)
{
    int tid;
    int request1[MAXR];
    int request2[MAXR];
    int claim[MAXR];

    tid = *((int*)a);
    rm_thread_started (tid);

    setarray(claim, NUMR, 10,10,10,10, 10);
    rm_claim (claim);

    setarray(request1, NUMR, 5, 3, 2, 4, 1);
    pr (tid, "REQ", NUMR, request1);
    rm_request (request1);

    sleep(4);

    setarray(request2, NUMR, 1, 1, 1, 2, 1);
    pr (tid, "REQ", NUMR, request2);
    rm_request (request2);

    rm_release (request1);
    rm_release (request2);

    rm_thread_ended();
    printf("Thread %d is ended! \n", tid);
    pthread_exit(NULL);
}

int main(int argc, char **argv)
{
    int i;
    int tids[NUMP];
    pthread_t threadArray[NUMP];
    int count;
    int ret;

    if (argc != 2) {
        printf ("usage: ./app avoidflag\n");
        exit (1);
    }

    AVOID = atoi (argv[1]);

    if (AVOID == 1) {
        rm_init (NUMP, NUMR, exist, 1);
    }    
    else {
        rm_init (NUMP, NUMR, exist, 0);
    }
    
    if(random_func) {
        for(int i = 0; i < NUMP; i++) {
            tids[i] = i;
            pthread_create (&(threadArray[i]), NULL,
                    (void *) randomFunc, (void *)
                            (void*)&tids[i]);
        }
    } else {
        i = 0;  // we select a tid for the thread
        tids[i] = i;
        pthread_create (&(threadArray[i]), NULL,
                        (void *) threadfunc1, (void *)
                                (void*)&tids[i]);

        i = 1;  // we select a tid for the thread
        tids[i] = i;
        pthread_create (&(threadArray[i]), NULL,
                        (void *) threadfunc2, (void *)
                                (void*)&tids[i]);

        i = 2;  // we select a tid for the thread
        tids[i] = i;
        pthread_create (&(threadArray[i]), NULL,
                        (void *) threadfunc3, (void *)
                                (void*)&tids[i]);

        i = 3;  // we select a tid for the thread
        tids[i] = i;
        pthread_create (&(threadArray[i]), NULL,
                        (void *) threadfunc4, (void *)
                                (void*)&tids[i]);

        i = 4;  // we select a tid for the thread
        tids[i] = i;
        pthread_create (&(threadArray[i]), NULL,
                        (void *) threadfunc5, (void *)
                                (void*)&tids[i]);
    }

    //no deadlock avoidence
    if(!AVOID) {
        count = 0;
        while ( count < 10) {
            sleep(1);
            rm_print_state("The current state");
            printf("\n");
            ret = rm_detection();
            fflush(stdout);
            if (ret > 0) {
                printf ("deadlock detected, count=%d\n", ret);
                rm_print_state("state after deadlock");
            }
            count++;
        }

        if (rm_detection() == 0) {
            for (i = 0; i < NUMP; ++i) {
                pthread_join (threadArray[i], NULL); 
                printf ("joined\n");
            }
        }
    } else {
        count = 0;
        while ( count < 10) {
            sleep(1);
            rm_print_state("The current state");
            printf("\n");
            fflush(stdout);
            count++;
        }

        for (i = 0; i < NUMP; ++i) {
            pthread_join (threadArray[i], NULL); 
            printf ("joined\n");
        }
    }
}

