...
int main(int argc, char** argv) {
    ...
    int sharedMemSize = n * k * sizeof(wordFreq);

    // set the size of the shared memory
    ftruncate(shm_fd, sharedMemSize);

    // create a pointer to the shared memory
    shrPtr = (wordFreq*) mmap(NULL, sharedMemSize, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
    wordFreq* original = shrPtr;

    // create child processes to process the input files
    pid_t pid;
    for (int i = 0; i < n; i++) {
        pid = fork();
        if (pid < 0) {
            ...
            // child process
            freqTable table = processFile(inputFiles[i]);
            //displayFreqTable(table);
            saveShared(table);
            
            //freqTable table = processFile(inputFiles[i]);
            //saveShared(table, sharedMemName, sharedMemSize / n);
            //displayFreqTable(table);
            exit(0);
        } else {
            //parent process
    ...
    mergeTables(original);

    // cleanup
    munmap(shrPtr, sharedMemSize);
    close(shm_fd);
